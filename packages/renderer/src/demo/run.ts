...
export type RunRendererDemoOptions = {
  // Which scene to spawn (the entry owns selection/persistence; switching = reload).
  scene: SceneName;
  // Called right after the shared GUI is created — the entry adds its scene switcher here.
  setupDemoGUI?: (gui: GUI) => void;
};

export async function runRendererDemo({ scene: SCENE, setupDemoGUI }: RunRendererDemoOptions) {
  ...
  // The entry's scene switcher goes first, above the voxel/GI folders.
  setupDemoGUI?.(gui);
