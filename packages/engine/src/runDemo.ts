...
function buildGui(world: EngineWorld, spawned: Spawned[], setupDemoGUI?: (gui: GUI) => void): GUI {
  ...
  // The entry's scene switcher goes first, above the spawn/VCT folders.
  setupDemoGUI?.(gui);
...
export type RunEngineDemoOptions = {
  // Called right after the GUI is created — the entry adds its scene switcher here.
  setupDemoGUI?: (gui: GUI) => void;
};

export async function runEngineDemo({ setupDemoGUI }: RunEngineDemoOptions = {}): Promise<void> {
